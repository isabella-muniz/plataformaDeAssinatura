from django.contrib.auth.base_ser import BaseUserManager
...
class CustemUserManager(BaseUserManager):
    def create_user(self, email: str, password: str, **other_fields):
        ...
        user = self.model(
            email = self.normalize_email(email), #passa o email de maiuscula para minuscula
            **other_fields,
        )
    ...
    def create_superuser(self, email: str, password: str, **other_fields):
        ...
        must_be_true_fields = ('is_staff', 'is_superuser', 'is_active')
            for field in must_be_true_fields:
                ...
                user = self.create_superuser(email, password, **other_fields)
                user.is_admin = True
                user.save()
                return user
    #:
